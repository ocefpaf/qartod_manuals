...
**Figure 2- 1. (Top)** This Pacific Marine Environmental Laboratory buoy was deployed in April 2018 in the mouth of the Chesapeake
...
**Figure 2- 2.** An underwater Slocum Webb glider (L) fitted with a prototype Sea-Bird Scientific ISFET-based pH sensor (photo
...
tests described for a fixed sensor may remain unchanged, or they are conducted along the vessel track ‘s' or
...
**Figure 2- 4.** Some commonly used pH sensors (photos courtesy of vendors).
...
```
Figure 2- 6. This pH time series is from a Sunburst SAMI-pH sensor deployed on the Ocean Station Papa mooring. Spikes of
...
- Florida Atlantic University Harbor Branch Indian River Lagoon Observatory,
    [http://fau.loboviz.com/](http://fau.loboviz.com/)
- National Estuarine Research Reserve System, [http://cdmo.baruch.sc.edu/get/realTime.cfm](http://cdmo.baruch.sc.edu/get/realTime.cfm)
...
presume a time ordered series of observations and denote the most recent observation as pH _n_ , preceded by a
value at pH _n- 1_ , and so on backwards in time. They were developed from input by authors and reviewers of
this manual, as well as from QARTOD workshops (QARTOD 2003-2009). The focus is primarily on the
real-time QC of observation pH _n_ , pH _n- 1_ , and pH _n- 2_. There are several instances when tests are closely related,
...
```
Data point _n- 1_ exceeds a selected threshold relative to adjacent data points.
...
```
- **Example 1:** The rate of change between pH _n- 1_ and pH _n_ must be less than three standard deviations
...
- **Example 2:** The rate of change between pH _n- 1_ and pH _n_ must be less than 0.1 +2SD.
**Flags Condition Codable Instructions**
...
observation of the same value. This test compares the present observation (pH n ) to a number
(REP_CNT_FAIL or REP_CNT_SUSPECT) of previous observations. pH n is flagged if it has the same value as
...
observations are equal, pH n is flagged
fail.
```
```
pH n ≠ 0
AND
For i=1,REP_CNT_FAIL pH n - pH n-I <EPS
...
```
```
For i=1,REP_CNT_SUSPECT pH n - pH n-I <EPS
```
```
Pass=1 Applies for test pass condition. N/A
Test Exception : None
...
```
|pH (^1) _n_ - pH (^1) _n- 1_ |>N_pH1_DEV*SD1
AND
|pH (^2) _n_ - pH (^2) _n- 1_ |<N_pH2_DEV*SD2
Pass=1 N/A N/A
**Test Exception:** None.
**Test specifications to be established locally by operator.
...
pp.44-60. DOI: [http://dx.doi.org/10.25607/OBP-](http://dx.doi.org/10.25607/OBP-) 18

Bushnell, M. (2005). Quality Control, Quality Assurance, and Quality Flags [Presentation at QARTOD III, 2-
4 November 2005]. NOAA/NOS/CO-OPS, 18 slides. DOI: [http://dx.doi.org/10.25607/OBP-](http://dx.doi.org/10.25607/OBP-) 390
...
technology. Retrieved from: [http://pubs.acs.org/doi/abs/10.1021/es504849s.](http://pubs.acs.org/doi/abs/10.1021/es504849s.)
...
[http://www.pmel.noaa.gov/co2/GOA-ON/GOA-ON_Plan_v1.0_April2014.doc.](http://www.pmel.noaa.gov/co2/GOA-ON/GOA-ON_Plan_v1.0_April2014.doc.)
...
[http://www.ioc-](http://www.ioc-)
unesco.org/components/com_oe/oe.php?task=download&id=42566&version=1.0&lang=1&format=1
...
QARTOD I-V Reports ( 2003 - 2009 ): Accessed June 9, 2019 at https://ioos.noaa.gov/ioos-in-action/qartod-
meetings/.
...
https://www.researchgate.net/publication/304106232_Measuring_pH_in_the_Arctic_Ocean_Colorimetr
ic_method_or_SeaFET
...
[http://pubs.water.usgs.gov/twri9A6/](http://pubs.water.usgs.gov/twri9A6/)
...
discuss the actual tests. [http://www.oceandatastandards.org/](http://www.oceandatastandards.org/)

The ocean data standards resource pool can be found at: [http://www.oceandatastandards.org/resourcepool-](http://www.oceandatastandards.org/resourcepool-)
mainmenu- 7.

[http://www.oceandatastandards.org](http://www.oceandatastandards.org) is the higher-level page (see menu to the right for subpages).

Argo Quality Control Manual can be found at:
[http://www.argodatamgt.org/content/download/341/2650/file/argo-quality-control-manual-V2.7.pdf.](http://www.argodatamgt.org/content/download/341/2650/file/argo-quality-control-manual-V2.7.pdf.)
...
Geological Survey Techniques and Methods 1-D3, 51 p. [http://pubs.water.usgs.gov/tm1d3](http://pubs.water.usgs.gov/tm1d3)
...
- USGS - [http://water.usgs.gov/owq/quality.html](http://water.usgs.gov/owq/quality.html)
- USGS - [http://pubs.usgs.gov/tm/2006/tm1D3/](http://pubs.usgs.gov/tm/2006/tm1D3/)
- USGS - [http://or.water.usgs.gov/pubs/WRIR01-4273/wri014273.pdf](http://or.water.usgs.gov/pubs/WRIR01-4273/wri014273.pdf)
- WOCE - https://www.nodc.noaa.gov/woce/
- NWQMC - [http://acwi.gov/monitoring/](http://acwi.gov/monitoring/)
